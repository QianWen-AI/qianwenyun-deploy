#!/bin/bash
# qianwenyun · Docker 后端 bootstrap
# 占位符：
...
exec >> "$LOG" 2>&1
echo "[$(date -u +%FT%TZ)] === qianwenyun docker bootstrap start ==="
...
DB_ENV_OPT=""
[ -f /etc/qianwenyun/db.env ] && DB_ENV_OPT="--env-file /etc/qianwenyun/db.env"
...
[Unit]
Description=qianwenyun app container
After=docker.service
Requires=docker.service

[Service]
Restart=always
ExecStartPre=-/usr/bin/docker rm -f qianwenyun-app
ExecStart=/usr/bin/docker run --rm --name qianwenyun-app -p ${BACKEND_PORT}:${BACKEND_PORT} ${DB_ENV_OPT} ${IMAGE_NAME}
ExecStop=/usr/bin/docker stop qianwenyun-app
...
UNIT
  ...
  if [ -f /etc/qianwenyun/db.env ]; then
    cp /etc/qianwenyun/db.env ./.env
  fi
